    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($item->getSku() == "sap_invoice") {
            ...
            if (isset($options['options'])) {
                ...
                $docEntry_array = array_filter($customOptions,
                    function ($var) {
                ...
                $amount_array = array_filter($customOptions,
                    function ($var) {
                ...
                $docType_array = array_filter($customOptions,
                    function ($var) {
                ...
                if($this->_helper->isValideInvoice($docEntry,$docType)) {
                    ...
                    if($requested_amount > $invoiceOpenBalance) {
                        $final_amount = $invoiceOpenBalance;
                        throw new \Magento\Framework\Exception\LocalizedException(__("Request amount is more than invoice open balance, Invoice open balance = %1",$invoiceOpenBalance));
                        return;
                    }
                    ...
                    if($requested_amount < $invoiceOpenBalance) {
                        if(!$this->_helper->allowPartialPayment()) {
                            throw new \Magento\Framework\Exception\LocalizedException(__("Partial payment is disabled for the moment, the amount set to the invoice open balance = %1",$invoiceOpenBalance));
                            return;
                        }else{
                            $final_amount = $requested_amount;
                        }
                    ...
                    if($final_amount == 0) {
                        throw new \Magento\Framework\Exception\LocalizedException(__("Amount is not valid for invoice %1.",$docEntry));
                        return;
                    }
                    ...
                    throw new \Magento\Framework\Exception\LocalizedException(__("Invoice %1 is not valid.",$docEntry));
                    return;
                }
            }
        }
